...
public class mainScreen extends javax.swing.JFrame {
    ...
    private void contBActionPerformed(java.awt.event.ActionEvent evt) {
        // TODO add your handling code here:
        if(amtShips == 5){
            if(homeScreen.isVsAi()) {
                close();
                firstPlayerVsAI o = new firstPlayerVsAI();
                o.setVisible(true);
            }
            else
            {
                coverFrame();
                JOptionPane.showMessageDialog(null, "Player 1 press enter and once the frame closes then Player 2 Choose your board");
                close();
                coverFrame.dispose();
    ...
    public void drawBoard(){
        ...
        for(int x = 0; x < 10; x++) {
            for (int y = 0; y < 10; y++) {
                if (board[x][y] == 1) new LabelWithPos(new JLabel(ship), 53 + (25 * x), 38 + (25 * y));
                else new LabelWithPos(new JLabel(no), 53 + (25 * x), 38 + (25 * y));
            }
        }
        for(LabelWithPos l : LabelWithPos.getRegistry()) {
